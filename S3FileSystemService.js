...
export default class S3FileSystemService extends FileSystemService {
	...
	static constructorProperties = {
		...
		awsServiceInstanceName: {
			...
			required: true,
			description:
				"The name of the configured AWSService instance in the registry.",
		},
	...
	constructor({
		...
		if (!registry) {
			throw new Error(
				"S3FileSystem constructor requires a 'registry' instance.",
			);
		}
		if (!bucketName) {
	...
	_getS3Client() {
		...
		if (!awsService || !(awsService instanceof AWSService)) {
			throw new Error(
				`AWSService instance '${this.awsServiceInstanceName}' not found or is not of the correct type.`,
			);
		}
	...
	async exists(fsPath) {
		...
		if (!s3Key) {
			// Path like '/' or '' results in empty key
	...
	async createDirectory(fsPath, options = {}) {
		...
		if (s3Key === "") {
			// Root directory always exists and cannot be "created" as an object
			return true;
		}
	...
	async rename(oldPath, newPath) {
		...
		throw new Error(
			"Method rename is not supported by S3FileSystem. Use copy() and deleteFile() to achieve a move operation.",
		);
	}
	...
	async glob(pattern, options = {}) {
		...
		throw new Error(
			"Method glob is not fully supported by S3FileSystem. Only prefix-based listing is available via getDirectoryTree.",
		);
	}

	async grep(searchString, options = {}) {
		throw new Error(
			"Method grep is not supported by S3FileSystem. Consider using S3 Select for specific use cases or downloading files for local search.",
		);
	}
}
